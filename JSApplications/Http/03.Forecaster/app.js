function attachEvents() {
        ...
        if (loc.name == inputo.value) {
            ...
            async function dayforecast(){
                ...
                if (document.querySelector(`div .forecasts`)){
                    divfore.replaceChild(divfo, document.querySelector(`div .forecasts`));
                } else {
            ...
            async function triday(){
             ...
             for (let k of restridata.forecast) {
              ...
              if (document.querySelector(`div .forecast-info`)){
                divupcome.replaceChild(divup, document.querySelector(`div .forecast-info`));
              } else {
    ...
    let endspan = document.createElement(`div`);
    endspan.classList.add(`forecasts`);
    endspan.innerText = `Error`;
    if (document.querySelector(`div .forecasts`)){
        divfore.replaceChild(endspan, document.querySelector(`div .forecasts`));
    } else {
        divfore.appendChild(endspan);
    }
    let enddiv = document.createElement(`div`);
    enddiv.classList.add(`forecast-info`);
    enddiv.innerText = ``;

    if (document.querySelector(`div .forecast-info`)){
        divupcome.replaceChild(enddiv, document.querySelector(`div .forecast-info`));
      } else {
        divupcome.appendChild(enddiv);
      }
